...
RegisterNetEvent('aj_dj:createMusicMenu', function()
    lib.registerContext({
        id = 'music_menu',
            ...
            options = {
                {
                    ...
                    event = 'a_dj:playMusicFromPlaylist'
                },
                {
...
RegisterNetEvent('a_dj:playMusicFromPlaylist', function (data)
    local input = data.music
    if input then
        TriggerServerEvent('a_dj:playMusic', input)
    end
end)

RegisterNetEvent('a_dj:playMusicMenu', function (YoutubeURL)
    local input = lib.inputDialog(Config.Language['songSel'], {Config.Language['url']})
    if input then
        local YoutubeURL = input[1]
        TriggerServerEvent('a_dj:playMusic', YoutubeURL)
    end
end)

RegisterNetEvent('a_dj:changeVolumeMenu', function ()
    local input = lib.inputDialog(Config.Language['musicVolume'], {Config.Language['musicVolumeNm']})
    if input then
        local volume = input[1]
        TriggerServerEvent('a_dj:changeVolume', volume)
    end
end)
            ...
            if CurrentAction ~= nil then
                ...
                if IsControlPressed(1, 38) then
                    ...
                    if CurrentAction == 'musicMenu' then
                        TriggerEvent('aj_dj:createMusicMenu')
                    end
                end
            end
            Wait(sleep)
